...
reductions below (`sum`, `prod`, `mean`, `std`, `max`, `min`, `argmax`,
`argmin`) additionally accept an `axis=` keyword when `x` is a `Mat`:
`axis=0` collapses each column to one number (a length-`cols` `Vec`),
`axis=1` collapses each row (a length-`rows` `Vec`), instead of flattening
the whole matrix to a single scalar.

| Function | Signature | Description |
|---|---|---|
| `sum`, `prod` | `sum(x)` | Sum / product of every element of `x` — a scalar, `Vec`, or `Mat`/`Signal` (real; `sum` additionally accepts a `CVec`/`CMat` and returns a `Complex` in that case). With no other argument, reduces to one scalar `Num` (or `Complex`). On a `Mat`, the optional `axis=` keyword (`0` or `1`) reduces along just that dimension instead, returning a `Vec` of length `cols` (`axis=0`) or `rows` (`axis=1`). Returns a scalar `Num` — a `Complex` where `sum` reduced a `CVec`/`CMat` — or that `Vec` under `axis=`. |
| `mean` | `mean(x)` | Arithmetic mean of every element of `x` — a scalar, `Vec`, or `Mat`/`Signal`. With no other argument, flattens `x` to one scalar `Num` (errors on an empty vector). On a `Mat`, the optional `axis=` keyword (`0` or `1`) computes the mean along just that dimension, returning a length-`cols` or length-`rows` `Vec` — see `row_mean` below for a dedicated column-`Mat`-returning version of the row case. Returns a scalar `Num` with no `axis=`, or that `Vec` with one. |
| `max`, `min` | `max(x)` or `max(a, b)` | One argument reduces `x` (a scalar, `Vec`, or `Mat`/`Signal`) to its largest/smallest element, a scalar `Num`. Two arguments `a`, `b` (matching or broadcastable shapes) instead compare elementwise and keep the larger/smaller of each pair — the "clip" idiom, e.g. `max(x, 0)`. In the one-argument form on a `Mat`, the optional `axis=` keyword (`0` or `1`) reduces along just that dimension instead, returning a length-`cols`/`rows` `Vec` rather than a single scalar. |
| `argmin`, `argmax` | `argmin(x)` | 0-based index of the first-occurring minimum/maximum element of `x` (a scalar, `Vec`, or `Mat`/`Signal`, flattened). Returns a scalar `Num` index. On a `Mat`, the optional `axis=` keyword (`0` or `1`) instead returns the index of the extremum *within* each column or row — a `Vec` of length `cols`/`rows`, each entry an index into that column/row rather than into the flattened matrix. |
| `median` | `median(x)` | Middle order statistic of `x` (a scalar, `Vec`, or `Mat`/`Signal`, flattened): the middle element for an odd-length input, the average of the two middle elements when `len(x)` is even. Returns a scalar `Num`; no `axis=` support. |
| `quantile` | `quantile(x, q)` | Linear-interpolation quantile (NumPy's default `interpolation="linear"` method). `x` is a scalar, `Vec`, or `Mat`/`Signal` (flattened); `q` is a scalar rank in `[0, 1]` (`0.5` is the median), interpolated between the two nearest order statistics when it doesn't land exactly on one. Returns a scalar `Num`. |
...
| `std` | `std(x)` | Sample standard deviation (N-1, unbiased, denominator) of every element of `x` — a scalar, `Vec`, or `Mat`/`Signal`. With no other argument, flattens `x` to one scalar `Num`. On a `Mat`, the optional `axis=` keyword (`0` or `1`) computes the standard deviation down each column or across each row instead, returning a length-`cols`/`rows` `Vec`. Returns a scalar `Num` with no `axis=`, or that `Vec` with one. |
| `var` | `var(x)` | Sample variance, same N-1 (unbiased) denominator as `std`, so `var(x) == std(x)^2` always holds. `x` is a scalar, `Vec`, or `Mat`/`Signal`, flattened. Unlike `std`, `var` does not currently accept an `axis=` keyword. Returns a scalar `Num`. |
