...
It requires exactly two colons (three parts) — `1:3` alone is not a range literal, since a bare `:` has no other meaning outside `[...]`. It shares `to`'s inclusive, float-tolerant endpoint semantics (§12), and since §15 bracket slicing is now inclusive too, the three-part `start:step:stop` reading means the same span inside brackets and out. The remaining difference is what they produce, not which elements they name: the literal builds a value, the slice selects from one.
...
### 17.2 Function values and lambdas

A function is a value. Its name, written without a call, **is** the
function:
...
Builtins are values too, so `sqrt` can be passed the same way `dbl` can.

A function with no name is written in the same two shapes a named one has,
with the name left out — one expression after `:=`, or a block:
...
For a single argument there is a third, terser spelling with no
parentheses, which reads best where the function is written exactly where
it is passed:
...
All three build the same thing, close over the same scope, and are the same
feature. `=>` and not `->`, because `->` already means reshape
(`x -> (2, 3)`) — borrowing it makes two different things look alike, and
did: hooking a lambda on `->` silently stopped `v -> (2, 3)` reshaping.

#### What a lambda sees

A **local** variable is captured by value where the lambda is written. That
is what makes a returned function still work after the call that made it has
finished:
...
add3 = make_adder(3)
print(add3(4))          # 7 -- `n` is gone, the value is not
```

A **module-level** variable is read when the lambda runs, not when it is
written — the same rule §17.0 already gives an ordinary function body:
...
An argument shadows a capture of the same name: a capture is what the
surrounding code had, a parameter is what this call was given.

#### Passing one

Anywhere a builtin takes "the function to call", it takes the function
itself. **The collection comes first**, in all of them:

```qu
print(map([1, 2, 3], dbl))              # (2, 4, 6)
print(map([1, 2, 3], (x) := x + 1))     # (2, 3, 4)
print(map([1, 4, 9], sqrt))             # (1, 2, 3)
print(filter([1, 2, 3, 4, 5], (x) := x > 2))
print(reduce([1, 2, 3, 4], (a, b) := a + b))
print(fzero((x) := x * x - 9, 1, 5))    # 3
```

Because the collection leads, the two ways Qu already has of chaining a
call both reach these for free:

```qu
xs = [1, 2, 3]
print(xs.map(dbl))                      # recv.f(args) is f(recv, args)
print(xs |> map(dbl))                   # `|>` fills the first slot
```

`map` used to take the function first, matching Python. It no longer does,
because `pmap` never did — the same word doing the same job in the opposite
order, with nothing in a call to say which one you were in. The old order
still runs and warns once per run, naming the fix; see §17.3.

The older spelling of the function itself — its name as a **string**,
`map(xs, "dbl")` — also still works, and still refuses builtins. A quoted
name is indistinguishable from a typo, so `map(xs, "sqrt")` stays an error;
written without quotes it is unambiguous, and allowed.

#### Where they can go

A function value is an ordinary value: it goes in a list, comes back from a
call, and can be called straight off whatever produced it.

```qu
fs = [(x) := x * 2, (x) := x * 3]
print(fs[1](10))                        # 30
print(make_adder(5)(6))                 # 11
```

A record whose fields are functions is a module written in Qu, and a field
holding a function is **called**, not passed as a receiver:

```qu
function make_ops(k)
    return {scale = (x) := x * k, shift = (x) := x + k}
end function

ops = make_ops(3)
print(ops.scale(5))                     # 15
```

A function is code, not data: it cannot be written to a `.json`, `.xml` or
any other file.

### 17.3 Argument order: the collection comes first

Every builtin that takes a function and something to run it over takes the
**collection first**:

```text
map(xs, f)        pmap(xs, f)       apply(x, f)
filter(xs, p)     find(xs, p)       all(xs, p)      any(xs, p)
reduce(xs, f)     fold(xs, f, initial)
```

Three reasons, and none of them is symmetry for its own sake:

- `recv.method(args)` means `method(recv, args)` throughout Qu, so
  `xs.map(f)` only exists if the collection is the first argument.
- `|>` puts the piped value in the first slot, so `signals |> map(f)` only
  composes if the collection is first.
- Every one of them except `map` already did it this way.

`map` used to take the function first, copying Python. `pmap` never did.
That left the language with two spellings of one idea in opposite orders
and no way to tell from a call site which one you were in — the kind of
thing that is fine until the day it is a silent wrong answer.

#### The old order still runs

Nothing written before this stops working. A function value — or the name
of a function that exists — is never a collection, so the two arguments can
always be told apart, whichever way round they were written:

```qu
dbl(x) := x * 2
xs = [1, 2, 3]
print(map(dbl, xs))     # runs, and prints a notice once per run
print(map(xs, dbl))     # the spelling to move to
```

The notice fires **once per builtin per run**, not once per call, and names
the fix. A quoted name that does not resolve is still an error rather than
a guess, in either order:

```text
map(names, "upper")   # error: `upper` is a builtin -- pass it without quotes
map("upper", names)   # the same error, about the same string
```
...
```qu
import dsp.filter
from dsp.fft import fft, ifft, fftshift
using plot
```
...
Names beginning with `_` are private by convention unless exported explicitly.
...
`else` runs only when the body raised nothing, and sits **outside** the
guarded region: an error raised in the `else` arm is not caught by that
same `try`. That is the point of having it — code which belongs after the
risky call, but must not be guarded by the same handler, or a failure in
the follow-up gets reported as a failure of the thing being tried.

`finally` runs on the way out however the block was left: normally, via a
caught error, via an error that is still propagating, or via a `return`
or `break` jumping out of the middle. That last case is the reason it
exists — cleanup written after the `end` is skipped by exactly the paths
that most need it. An in-flight `return` is set aside while the cleanup
runs and restored afterwards, so its value still comes back; an error
raised *inside* the cleanup wins over the one it was cleaning up after,
since hiding a second fault behind the first helps nobody.

**A `try` with no `catch` does not swallow.** The error runs the `finally`
if there is one and then keeps propagating. Discarding an error because a
handler was not written would be exactly the silent failure this language
is organised against; `unsafe` is the construct for "report it and carry
on".
...
A catalogue of call shapes rather than a program: the paths are
illustrative, so it is marked not-runnable. `read_json`/`write_json` were
listed here and **do not exist** — JSON is read with `parse_json` on text
you already have, and written with `jsonify`.
...
cfg = parse_json(read_all_text("config.json"))
write_text("result.json", jsonify(result))
```
...
Call shapes against an illustrative path, so not run by the build:
...
There is an anonymous form too — the same line with the name left out —
and `map`/`reduce`/`filter` take the function itself, not its name as a
string. See §17.2.

```qu
square(x) := x^2
print(map([1,2,3,4], square))       # (1, 4, 9, 16)
print(map([1,2,3,4], (x) := x^2))   # (1, 4, 9, 16) -- no name needed
print(map([1,2,3,4], "square"))     # (1, 4, 9, 16) -- the old string form
```
